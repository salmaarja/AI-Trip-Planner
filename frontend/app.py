...
def generate_itinerary():
    ...
    try:
        with st.status("Building your AI itinerary…", expanded=True) as status:
            st.write("🧠 Step 1/4: Understanding your trip profile (party, budget, interests)")
            st.write("🗺️ Step 2/4: Generating itinerary with local AI (Ollama)")
            r = requests.post(f"{API_BASE}/trip/build", json=payload, timeout=600)

            st.write("✅ Step 3/4: Validating and formatting the itinerary")
            st.write("🎁 Step 4/4: Preparing your final view")
        ...
        st.error("The AI took too long. Try fewer days or check Ollama is running.")
    except Exception as e:
        st.error(f"Unexpected error: {e}")


def activity_card(a: dict):
    # small “card-like” UI using Streamlit containers
    with st.container(border=True):
        st.markdown(f"**{a['start_time']}–{a['end_time']} • {a['title']}**")
        meta = f"{(a.get('category') or '').title()}"
        if a.get("location"):
            meta += f" • {a['location']}"
        st.caption(meta)
...
def show_itinerary(data: dict):
    # Header row: title + destination image if present
    left, right = st.columns([3, 2])
    with left:
        st.subheader(data.get("title", "Itinerary"))
        if data.get("notes"):
            st.info(data["notes"])
    with right:
        if data.get("destination_image_url"):
            st.image(data["destination_image_url"], use_container_width=True)

    # AI explanations (Gap #1 shown nicely)
    if data.get("explanations"):
        st.markdown("### Why this itinerary?")
        for x in data["explanations"]:
            st.write(f"• {x}")

    st.divider()

    # Day tabs (clean, app-like)
    days = data.get("days", [])
    if not days:
        st.warning("No days found in itinerary.")
        return

    tab_labels = [f"Day {d['day_index']} • {d['date']}" for d in days]
...
with tab_plan:
    ...
    if st.session_state.step == 0:
        ...
        with colB:
            ...
            st.caption("Generated by local AI (Ollama) and formatted into a structured plan.")
